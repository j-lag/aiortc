...
def decoder_worker(loop, input_q, output_q, recorder):
	codec_name = None
	decoder = None
	...
	while True:
		...
		if recorder is not None:
			recorder.record(codec, encoded_frame)
		else:
...
class RTCRtpReceiver:
	...
	async def receive(self, parameters: RTCRtpReceiveParameters, recorder):
		"""
		...
		if not self.__started:
			...
			self.__decoder_thread = threading.Thread(
				...
				args=(asyncio.get_event_loop(), self.__decoder_queue, self._track._queue, recorder))
