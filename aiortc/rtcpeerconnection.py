...
class RTCPeerConnection(EventEmitter):
	...
	def __init__(self, configuration=None):
		...
		self.__videoPacketRecorder = self.__configuration.videoPacketRecorder
		self.__audioPacketRecorder = self.__configuration.audioPacketRecorder
		self.__videoPacketReplay = self.__configuration.videoPacketReplay
		self.__audioPacketReplay = self.__configuration.audioPacketReplay
			...
			if iceTransport.iceGatherer.getLocalCandidates() and transceiver in self.__remoteIce:
				...
				if dtlsTransport.state == 'connected':
					if transceiver.currentDirection in ['sendonly', 'sendrecv']:
						replay = self.__videoPacketReplay if transceiver._RTCRtpTransceiver__kind == 'video' else self.__audioPacketReplay
						await transceiver.sender.send(self.__localRtp(transceiver),replay)
					if transceiver.currentDirection in ['recvonly', 'sendrecv']:
						recorder = self.__videoPacketRecorder if transceiver._RTCRtpTransceiver__kind == 'video' else self.__audioPacketRecorder
						await transceiver.receiver.receive(self.__remoteRtp(transceiver), recorder)
		if self.__sctp:
