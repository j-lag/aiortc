...
class H264Encoder:
	...
	#@staticmethod
	#def _split_bitstream(buf):
	#	# rewrited in a more pytonic way for performance improvement: 
	#	# cProfile says was 60 % of "encode" time, now < 1%
	#	i = 0
	#	nal_0x0000 = b'\x00\x00'
	#	nal_0x000001 = b'\x00\x00\x01'
	#	hasDataToProcess = True
	#	while hasDataToProcess:
	#		#find start 
	#		i = buf.find(nal_0x000001, i)
	#		if i != -1:
	#			i = i + 3
	#		else:
	#			return
	...
	#
	#		nal_end = None
	#		while nal_end is None:
	#			i = buf.find(nal_0x0000, i)
	#			if i == -1:
	#				#end of buffer
	#				nal_end = len(buf)
	#				hasDataToProcess = False
	#			elif buf[i + 2] == 0x00 or buf[i + 2] == 0x01:
	#				#found it!
	#				nal_end = i
	#			else:
	#				#try again...
	...
	def _encode_frame(self, frame, force_keyframe):
		try:
			...
			print("error in _encode_frame", ex)
